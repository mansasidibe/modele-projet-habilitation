...
jQuery(document).ready(function ($) {
    ...
    const get_bondecommande_by_id = async (id) => {
        console.log("id:", id);

        const response = await axios.get(`${getBaseURL()}api/v1/commercial/bondecommandes/${parseInt(id)}`, {
            headers: {
    ...
    let get_bondecommande_id = parseInt(document.getElementById('id').value);
    get_bondecommande_by_id(get_bondecommande_id)
        .then(data => {
            console.log("data:", data);
            $('#code_ngser').val(data[0].code_ngser);
            ...
            data[0]?.media.map(row => {
                ...
                if (row.statut === 1) {
                    ...
                    $('#existant').val(true);
                    var elements = document.getElementById('chargementdefichier');
                ...
                if (data[0].code == null || data[0].code == "") {
                    var elements = document.getElementById('chargementdefichier');
...
const sub = async (elt) => {
    ...
    if (files.length > 0) {
        ...
        try {
            const response = await axios.post(getBaseURL() + 'api/v1/commercial/imageStore',
                {
                    fichier: files[0],
                    id: id,
                    code: code
...
var codereference = document.getElementById('code');
